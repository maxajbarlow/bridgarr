...
class ZileanScraper(BaseScraper):
    ...
    def _parse_result(self, item: dict, title_context: str) -> Optional[TorrentResult]:
        ...
        try:
            ...
            # Convert size to int (Zilean returns it as string)
            size = int(size_raw) if size_raw else 0
            seeders = item.get("seeders")

            # Zilean provides resolution directly, fallback to parsing from title
            quality = item.get("resolution") or self._parse_quality(raw_title)
